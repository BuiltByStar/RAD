...
export function PageShellHero({
  ...
  const minimalHero = variant === "roster";

  return (
    <section
      className={cn(
        "relative isolate overflow-hidden border-b border-neutral-900 bg-black",
        !minimalHero && "rad-dot-surface border-white/10 bg-[#050102]"
      )}
    >
      {heroVideo ? (
        <video
        ...
        <Image
          ...
          fill
          sizes="100vw"
          className={cn("z-[-3] object-cover", minimalHero ? "opacity-12" : "opacity-24")}
        />
      )}
      {!minimalHero ? (
        ...
        <div aria-hidden className="absolute inset-0 z-[-2] bg-[linear-gradient(180deg,rgba(229,6,47,0.08),transparent_38%,#000_88%)]" />
      )}
      {!minimalHero ? (
      ...
      <Container size="xl">
        <motion.div
          ...
          className={
            compact
              ? "grid gap-6 pb-6 pt-16 sm:pb-7 sm:pt-20 lg:pb-8 lg:pt-22"
        ...
        >
          <div className="min-w-0 max-w-4xl">
            ...
            <motion.div
              ...
              className={compact ? "mt-5 flex flex-wrap items-center gap-3" : "mt-7 flex flex-wrap items-center gap-3"}
            >
              {status ? (
